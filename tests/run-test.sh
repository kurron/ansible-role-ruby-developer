...
cp -R ~/GitHub/ansible-role-ruby-developer /tmp/roles/kurron.ruby-developer
...
CMD="docker run --env DISPLAY=unix${DISPLAY} \
                --env HOME=${HOME_DIR} \
                --env RUBYMINE_JDK=/usr/lib/jvm/azul-jdk-8 \
                ...
                kurron/ansible-role-ruby-developer:latest bash"
echo $CMD
$CMD
